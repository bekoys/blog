...
function __autoload($class) {
	...
	if (file_exists(SITE_PATH . '/includes/model/' . $class . '.php')) {
		require_once(SITE_PATH . '/includes/model/' . $class . '.php');
	} elseif (file_exists(SITE_PATH . '/includes/lib/' . $class . '.php')) {
		require_once(SITE_PATH . '/includes/lib/' . $class . '.php');
	} elseif ($class == '_markdownextra_tmpimpl') {
        require LIB_PATH . 'markdown.php';
    } else {
		fyMsg($class . '加载失败。');
	}
}
...
function checkPlugin($plugin) {
	if (is_string($plugin) && preg_match("/^[\w\-\/]+\.php$/", $plugin) && file_exists(SITE_PATH . '/content/plugins/' . $plugin)) {
		return true;
	} else {
		return false;
	}
...
function uploadFile($fileName, $errorNum, $tmpFile, $fileSize, $type, $isIcon = false, $is_thumbnail = true) {
	...
	switch ($result) {
		case '100':
			fyMsg('文件大小超过系统' . ini_get('upload_max_filesize') . '限制');
			break;
		case '101':
			fyMsg('上传文件失败,错误码：' . $errorNum);
			break;
		case '102':
			fyMsg('错误的文件类型');
			break;
		case '103':
			...
			fyMsg("文件大小超出{$ret}的限制");
			break;
		case '104':
			fyMsg('创建文件上传目录失败');
			break;
		case '105':
			fyMsg('上传失败。文件上传目录(content/uploadfile)不可写');
			break;
		default:
			return $result;
			break;
...
function emZip($orig_fname, $content) {
	...
	$zip = new ZipArchive();
	$tempzip = SITE_PATH . '/content/cache/emtemp.zip';
 ...
 */
function fyMsg($msg, $url = 'javascript:history.back(-1);', $isAutoGo = false) {
	if ($msg == '404') {
		header("HTTP/1.1 404 Not Found");
		$msg = '抱歉，你所请求的页面不存在！';
	}
...
{
    return _MarkdownExtra_TmpImpl::defaultTransform($text);
}


function escape_data($str) {
    if (! get_magic_quotes_gpc()) {
 ...
 * @param $data
 */
function fyAlert($data) { ?>
<script type='text/javascript'>
    $(function(){
        $("<div>" + "<?= $data ?>" + "</div>").dialog();  // !!! "" surrounding the php tag is a must
    });
</script>
<?php }
